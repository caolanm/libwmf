...
void gdImageDestroy(gdImagePtr im)
{
	...
	if(im->clipping_rects) {
	        free(im->clipping_rects);
	}
	free(im);
}
...
void gdImageArc(gdImagePtr im, int cx, int cy, int w, int h, int s, int e, int color)
	{
	gdPoint points[3];
	gdImageCArc(im, cx, cy, w, h, s, e, color,points);
	}
...
gdImagePtr
gdImageCreateFromXbmData(char *data,int w,int h)
{
    gdImagePtr im;
    int bit;
    int bytes;
...
gdImageCreateFromXbm(FILE *fd)
{
	...
	fread((void *)data,1,pos,fd);
	im = gdImageCreateFromXbmData(data,w,h);
	free(data);
	return(im);
}
