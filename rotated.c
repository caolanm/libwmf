...
static int              XRotPaintAlignedText();
...
int XRotDrawString(dpy, font, angle, drawable, gc, x, y, str)
    ...
    char *str;
{
	fprintf(stderr,"drawing rotated string angle %f %d %d %s\n",angle,x,y,str);
...
static int XRotPaintAlignedText(dpy, font, angle, drawable, gc, x, y, text,
				  align, bg)
    Display *dpy;
    XFontStruct *font;
    float angle;
    ...
    int bg;
{
	...
	fprintf(stderr,"rotate called with %f %d %d %s\n",angle,x,y,text);
    ...
    int align;
{
	...
	int delta;
	int lpDx[20] = {20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20,20};
	...
	pitem = items = (XTextItem *)malloc( strlen(text) * sizeof(XTextItem) );
	...
	if( lpDx ) /* explicit character widths */
		...
		while (i < strlen(text))
			...
			* becomes  non-zero */
			...
			do
				{
				delta = lpDx[i] - XTextWidth( font, text + i, 1);
				fprintf(stderr,"the delta from the last char is %d %x\n",lpDx[i],lpDx[i]);
				pitem->nchars++;
				} while ((++i < strlen(text)) && !delta);
	...
	for(i=((xl<0)?0:(int)xl); 
	    ...
	    /* rotate coordinates */
	    it=nint(item->cols_in/2 + ( di*cos_angle + dj*sin_angle));
	    jt=nint(item->rows_in/2 - (-di*sin_angle + dj*cos_angle));
